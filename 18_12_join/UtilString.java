...
public class UtilString {
    ...
    public static String entreComes(int[] numeros, char separador) {
        ...
        for (int i = 0; i < numeros.length; i++) {
            // Guardar en arraySeparat el element de l'array numeros en la posició de i
            arraySeparat += numeros[i];
            // Si i es diferent a la longitud numeros -1
            if (i != numeros.length-1) {
                // Guardar el separador més un espai
                arraySeparat += separador + " ";            
            }
        }
        // Retorna String arraySeparat con l'elements de l'array numeros separat per el separador
        return arraySeparat;
    }
    
    public static int valorEnter(String valor) {
        // Fer un while mentre el valor no sigui enter
        while (!UtilString.esEnter(valor)) {
            // Demanar el valor enter
            System.out.println("Per favor, un valor enter");
            // Llegir el valor
            valor = Entrada.readLine();
        }
        // Retornar el valor convertit a enter
        return Integer.parseInt(valor);
    }
    
    public static char separadorArray(String separador) {
        // Si el separador no està buit
        if (separador.isBlank()) {
            // Retonar com separador la coma
            return ',';
        // Del contrari
        } else {
    ...
    public static int quantsParaules(String text) {
        ...
        // Retornar quants paraules hi ha en el nouText
        return quants;
    }
    ...
    public static int quantsEspais(String text) {
        ...
        for (int i = 0; i < nouText.length(); i++) {
            ...
            if (Character.isWhitespace(c)) {
                // Guardar l'espai en blancs
                blancs += c;
            // Del contrari, si blancs no està buit
            } else if (!blancs.isEmpty()) {
                // Augmentar en 1 quants
                quants++;
                // Reiniciar blancs
    ...
    public static String junta(String[] cadenes, String separador) {
        // Declarar e inicialitzar el String nouCadena buit
        String nouCadena = "";
        // Fer un for per iterar l'array cadenes
    ...
    public static String junta(String[] cadenes, String separador, String darrerSeparador) {
        ...
        // Declarar e inicialitzar el String nouCadena buit
        String nouCadena = "";
        // Fer un for per iterar el array cadenes
        for (int i = 0; i < cadenes.length; i++) {
